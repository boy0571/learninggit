...
query = """ SELECT * FROM "all_test_ready_data" """
data = pd.read_sql(query, con=connection)
'''
inputpath='~/Documents/pankoo_04/all_test_ready_data_0531_copy.csv'
...
for i in range(1,len(data_shop)-56):
    data_shop.loc[i,'target_28']=data_shop.loc[i+29:i+56,'Record_Count'].mean()
    data_shop.loc[i,'holiday_28']=data_shop.loc[i+29:i+56,'holiday'].mean()
    data_shop.loc[i,'comp_28']=data_shop.loc[i+29:i+56,'comp'].mean()
    data_shop.loc[i,'brand1_store_count_28']=data_shop.loc[i+29:i+56,'brand_1_STORE_CODE_Count'].mean()
    data_shop.loc[i,'brand2_store_count_28']=data_shop.loc[i+29:i+56,'brand_2_STORE_CODE_Count'].mean()
    data_shop.loc[i,'brand6_store_count_28']=data_shop.loc[i+29:i+56,'brand_6_STORE_CODE_Count'].mean()
    data_shop.loc[i,'temp_mean_2week']=data_shop.loc[i+1:i+14,'temp'].mean()
    data_shop.loc[i,'package_count_28']=data_shop.loc[i+29:i+56,'PACKAGE_TODAY'].mean()
    data_shop.loc[i,'takeout_count_28']=data_shop.loc[i+29:i+56,'TAKEOUT_TODAY'].mean()
    data_shop.loc[i,'SP_count_28']=data_shop.loc[i+29:i+56,'SP_TODAY'].mean()
    data_shop.loc[i,'Spring_Eve_In']=data_shop.loc[i+29:i+56,'Spring_Eve'].max()
    data_shop.loc[i,'menu_photo_28']=data_shop.loc[i+29:i+56,'photo'].mean()
    data_shop.loc[i,'menu_recomm_28']=data_shop.loc[i+29:i+56,'recomm'].mean()
...
outputpath='~/Documents/pankoo_04/all_test_ready_data_skip_month_0531_new.csv'
#outputpath='C:\Users\derek\Desktop\\test_data\\all_test_ready_data_skip_month_2.csv'
